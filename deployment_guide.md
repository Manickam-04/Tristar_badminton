...
3. Clone your GitHub repository:
   ...
   ```
   *(This will clone the code into `/home/tristarbadminton/Tristar_badminton/`)*.
   *(Note: Your PythonAnywhere username is `tristarbadminton`)*.
...
1. Create a virtual environment using Python 3.10 (recommended):
   ...
   ```
   *(This creates and activates a virtual environment located at `/home/tristarbadminton/.virtualenvs/tristar-env`)*.
2. Install the application dependencies:
   ```bash
   pip install -r /home/tristarbadminton/Tristar_badminton/requirements.txt
   ```
...
3. Under the **Virtualenv** section on the Web tab, enter the path to the environment you created:
   `/home/tristarbadminton/.virtualenvs/tristar-env`
4. Under the **Code** section, locate the link to your **WSGI configuration file** (it looks like `/var/www/tristarbadminton_pythonanywhere_com_wsgi.py`). Click to edit it.
...
project_home = '/home/tristarbadminton/Tristar_badminton'
if project_home not in sys.path:
    sys.path.insert(0, project_home)

# 2. Inject environment variables for database path, sessions, and credentials
os.environ['DATABASE_PATH'] = '/home/tristarbadminton/Tristar_badminton/badminton.db'
...
2. Under **Scheduled Tasks**, set the following configuration:
   ...
   - **Command**: Enter the absolute path to your virtual environment's python interpreter and call the database backup command:
     ...
     ```
3. Click **Create**. This cron task will run daily, creating copies in `/home/tristarbadminton/Tristar_badminton/backups/` and enforcing a 7-day retention limit automatically.
...
2. Activate your virtual environment and run the interactive recovery script:
   ```bash
   workon tristar-env
   cd /home/tristarbadminton/Tristar_badminton/
   python restore.py
   ```
...
2. Delete the active database and backup files:
   ```bash
   rm /home/tristarbadminton/Tristar_badminton/badminton.db
   rm -rf /home/tristarbadminton/Tristar_badminton/backups/
   ```
3. Initialize the database schema completely fresh:
   ```bash
   workon tristar-env
   python /home/tristarbadminton/Tristar_badminton/database.py
   ```
...
* **Static Files not rendering**: By default, uWSGI serves static files efficiently. If styling or images fail to render, configure PythonAnywhere to serve them directly:
  ...
  - Add a path entry:
    - **URL**: `/static/`
    - **Directory**: `/home/tristarbadminton/Tristar_badminton/static`
  - Reload the web app.
